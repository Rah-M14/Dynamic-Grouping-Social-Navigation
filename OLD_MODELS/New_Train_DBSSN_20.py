...
class DGSN_Env(gym.Env):
    def __init__(self, grid_size=20, max_static_obstacles=30, max_dynamic_obstacles=30, max_steps=4000):
    ...
    # You can optionally handle the 'seed' or other kwargs if needed
        ...
        # Check if the goal_pos is the same as any static_obstacle
        while any(np.array_equal(self.goal_pos, obs) for obs in self.static_obstacles):
            print(' \n Found clasing Positions! Chaning the Goal Pos! \n')
            self.goal_pos = np.array(np.random.randint(0, self.grid_size, size=2)).astype('float')
...
save_path = os.path.join('Train', 'Saved_Models', 'Scan3_PPO')

combined_callback = CustomCallback(env=env, render_freq=1, save_freq=100, save_path=save_path, verbose=1)
model = PPO("MultiInputPolicy", env, verbose=1, tensorboard_log=log_path, device='cuda', n_epochs=1000000)
