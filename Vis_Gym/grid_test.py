...
class DGSN_Env(gym.Env):
    ...
    def _get_obstacles_positions(self):
        ...
        print(f"_get_obstacles_positions called, returning keys: {obstacles_positions.keys()}")
    ...
    def step(self, action):
        ...
        for i, obstacle in enumerate(self.dynamic_obstacles):
            obstacle_key = str(i+1)
            if obstacle_key in obstacles_positions:
                if len(obstacle['path']) > 1:
                    obstacle['current'] = obstacles_positions[obstacle_key]
            else:
                print(f"Warning: Key {obstacle_key} not found in obstacles_positions\n dynamic ")
